...
import {
  SA_CV_CHECKLIST_FILENAME,
  SA_CV_CHECKLIST_PATH,
} from '@/lib/sa-cv-checklist';
...
Here is your copy of The South African CV Checklist:
${pdfUrl}

It is a self-audit, not a rewrite. Work through the fifteen checks, tick a green, amber, or red light for each, then count your reds at the end. That number tells you how much work your CV needs, and what to do next.

If you would like a professional set of eyes on your CV, that is what I do. I offer CV reviews and full revamps, built for the South African market and the way recruiters here actually read:
${workUrl}
  ...
  <div style="margin:0;padding:32px 0;background:#f7f3ef;font-family:Raleway,Arial,Helvetica,sans-serif;color:#142334;">
    <div style="max-width:720px;margin:0 auto;background:#ffffff;border:1px solid #d8c8bb;">
      <div style="padding:40px;background:#142334;color:#ffffff;">
        <div style="font-size:12px;letter-spacing:0.24em;text-transform:uppercase;color:#c9ad98;font-weight:700;">The South African CV Checklist</div>
        <h1 style="margin:18px 0 0;font-family:Georgia,'Times New Roman',serif;font-size:42px;line-height:1.04;font-weight:500;">Your CV self-audit is ready.</h1>
        <p style="margin:18px 0 0;font-size:17px;line-height:1.65;color:#e4d8cb;">Fifteen checks that decide whether your CV gets seen, scored with a simple green, amber, red system in about ten minutes.</p>
      </div>
      <div style="padding:36px 40px 40px;">
        <p style="margin:0 0 18px;font-size:17px;line-height:1.7;">Hi ${escapeHtml(firstName)},</p>
        <p style="margin:0 0 24px;font-size:16px;line-height:1.75;color:rgba(20,35,52,0.78);">Work through the fifteen checks, tick a light for each, then count your reds at the end. That number tells you exactly where to start.</p>
        ...
        <p style="margin:28px 0 0;font-size:16px;line-height:1.75;color:rgba(20,35,52,0.78);">If you would like a professional set of eyes on your CV, I offer reviews and full revamps built for the South African market.</p>
...
export async function POST(request: Request) {
  try {
    ...
    const source = String(payload.source || 'sa-cv-checklist').trim().slice(0, 120);
    ...
    const pdfUrl = `${siteUrl}${SA_CV_CHECKLIST_PATH}`;
    ...
    const [, deliveryResult, , sourceLead] = await Promise.all([
      ...
      sendTransactionalEmail({
        ...
        subject: 'Your South African CV Checklist PDF',
      ...
      sendTransactionalEmail({
        ...
        text: `A visitor requested ${SA_CV_CHECKLIST_FILENAME}.
...
`,
      ...
      upsertSourceLead({
        ...
        metadata: {
          ...
          asset: SA_CV_CHECKLIST_FILENAME,
        },
      }),
      recordDashboardNotification({
        eventType: 'lead_magnet_download',
        source: 'sa-cv-checklist',
        title: `New lead magnet download - ${firstName}`,
        description: `${firstName} requested ${SA_CV_CHECKLIST_FILENAME}. Source: ${source}.`,
        contactName: firstName,
        contactEmail: email,
        href: `mailto:${email}?subject=${encodeURIComponent('Your South African CV Checklist download')}`,
        metadata: {
          asset: SA_CV_CHECKLIST_FILENAME,
          source,
          pdfUrl,
