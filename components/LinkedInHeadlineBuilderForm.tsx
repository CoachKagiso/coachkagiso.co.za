...
type LinkedInHeadlineBuilderFormProps = {
  source: string;
  compact?: boolean;
};

export default function LinkedInHeadlineBuilderForm({
  source,
  compact = false,
}: LinkedInHeadlineBuilderFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/lead-magnets/linkedin-headline-builder', {
        method: 'POST',
        headers: {
      ...
      setPdfUrl(body.pdfUrl || '/api/lead-magnets/linkedin-headline-builder/pdf');
      setState('success');
  ...
  return (
    <form onSubmit={handleSubmit} className={compact ? 'mt-6 grid gap-3' : 'mt-8 grid gap-3'}>
      ...
      {message && (
        ...
        >
          ...
          <span>
            {message}{' '}
            {pdfUrl && (
              <a href={pdfUrl} className="font-semibold text-[#142334] underline decoration-[#C9AD98] underline-offset-4">
                Download now
              </a>
            )}
          </span>
      ...
      <p className="mt-2 text-[12px] leading-relaxed text-[#142334]/50">
        By submitting, you agree to receive the PDF and occasional career notes from Coach Kagiso.
        POPIA-conscious, and you can unsubscribe at any time.
      </p>
    </form>
  );
}
