...
type First90DaysChecklistFormProps = {
  source: string;
  compact?: boolean;
};

export default function First90DaysChecklistForm({
  source,
  compact = false,
}: First90DaysChecklistFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/lead-magnets/first-90-days-checklist', {
        method: 'POST',
        headers: {
      ...
      setPdfUrl(body.pdfUrl || '/api/lead-magnets/first-90-days-checklist/pdf');
      setState('success');
