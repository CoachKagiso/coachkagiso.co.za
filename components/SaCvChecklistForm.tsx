...
import { SA_CV_CHECKLIST_PATH } from '@/lib/sa-cv-checklist';

type FormState = 'idle' | 'submitting' | 'success' | 'error';

type SaCvChecklistFormProps = {
  source: string;
  compact?: boolean;
};

export default function SaCvChecklistForm({
  source,
  compact = false,
}: SaCvChecklistFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/lead-magnets/sa-cv-checklist', {
        method: 'POST',
        headers: {
      ...
      setPdfUrl(body.pdfUrl || SA_CV_CHECKLIST_PATH);
      setState('success');
