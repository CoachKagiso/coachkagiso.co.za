...
import { CalendarClock, Check, FileText, Loader2, Pencil, Save, StickyNote, X } from 'lucide-react';
...
export default function ClientStrategyContext({ client, adminKey }: { client: ClientRecord; adminKey: string }) {
  ...
  const bookingTime = formatDateTime(
    intake.sourceMetadata.startTime || intake.sourceMetadata.sessionDate,
  );
  ...
  return (
    <section className="rounded-[8px] bg-[#142334] p-5 text-white" aria-labelledby="client-context-title">
      ...
      {bookingTime && (
        <div className="mt-5 flex flex-wrap items-center justify-between gap-4 rounded-[10px] border border-[#C9AD98]/70 bg-[#E8D7C5] px-4 py-3.5 text-[#142334]">
          <div className="flex items-center gap-3">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#142334] text-[#E8D7C5]">
              <CalendarClock className="h-5 w-5" />
            </span>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#6F584A]">Scheduled session</p>
              <p className="mt-0.5 text-[19px] font-semibold leading-tight tracking-[-0.01em]">{bookingTime} SAST</p>
            </div>
          </div>
          <span className="rounded-full bg-[#142334] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-[#E8D7C5]">Upcoming</span>
        </div>
      )}
