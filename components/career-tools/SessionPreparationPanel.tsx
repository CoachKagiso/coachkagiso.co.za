  ...
  return (
    <section className="grid gap-4">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 rounded-[8px] bg-[#142334] p-5 text-white">
          <div className="rounded-[8px] bg-white p-4 text-[#142334]"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#8C7466]">Session focus</p><p className="mt-2 font-serif text-[25px] leading-tight">{preparation.content.sessionFocus}</p><p className="mt-3 text-[14px] leading-relaxed text-[#142334]/72">{preparation.content.openingFrame}</p></div>
          <div className="grid gap-4 lg:grid-cols-2">
            <div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-white/55">Conversation flow</p><div className="mt-3 grid gap-2">{preparation.content.conversationFlow.map((step) => <div key={step.stage} className="rounded-[8px] bg-white/10 p-3"><p className="text-[13px] font-bold">{step.stage}</p><p className="mt-1 text-[12px] leading-relaxed text-white/70">{step.purpose}</p></div>)}</div></div>
            <div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-white/55">Priority questions</p><div className="mt-3 grid gap-2">{preparation.content.priorityQuestions.map((item) => <div key={item.question} className="rounded-[8px] bg-white/10 p-3"><p className="text-[13px] font-bold">{item.question}</p><p className="mt-1 text-[12px] leading-relaxed text-white/70">{item.whyItMatters}</p></div>)}</div></div>
          </div>
          <div className="grid gap-3 md:grid-cols-3">{([{ title: 'Listen for', items: preparation.content.listenFor }, { title: 'Close with', items: preparation.content.closeWith }, { title: 'Coach notes', items: preparation.content.coachNotes }]).map((section) => <div key={section.title} className="rounded-[8px] border border-white/10 bg-white/[0.06] p-3"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-white/55">{section.title}</p><ul className="mt-2 grid gap-2 text-[12px] leading-relaxed text-white/80">{section.items.map((item) => <li key={item}>• {item}</li>)}</ul></div>)}</div>
        </div>
      )}
    </section>
  );
