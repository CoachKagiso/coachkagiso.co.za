...
/**
 * Shared with the workspace header pill so both surfaces read the same booking time the same
 * way, instead of the header re-deriving it from a slightly different code path.
 */
export function formatClientBookingTime(sourceMetadata: Record<string, unknown> | null | undefined): string | null {
  const value = sourceMetadata?.startTime ?? sourceMetadata?.sessionDate;
...
/**
 * The header pill's version of the above: it should read as "your next session," so a date
 * that has already passed formats to null instead of sitting there stale until someone books
 * a new one.
 */
export function formatUpcomingClientBookingTime(sourceMetadata: Record<string, unknown> | null | undefined): string | null {
  const value = sourceMetadata?.startTime ?? sourceMetadata?.sessionDate;
  if (typeof value !== 'string' || !value) return null;
  const date = new Date(value);
  if (Number.isNaN(date.getTime()) || date.getTime() < Date.now()) return null;
  return formatClientBookingTime(sourceMetadata);
}
