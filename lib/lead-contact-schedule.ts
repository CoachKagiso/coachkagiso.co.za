...
export function getFollowUpScheduleAfterSend(
  ...
  if (isFollowUpOneTemplate(templateId)) {
    return {
      ...
      next_follow_up_at: addDaysAsDateString(
        now,
        source === 'first_90_days' || source === 'linkedin_headline' || source === 'cv_checklist' || source === 'interview_prep' ? 5 : 6
      ),
    };
  }
    ...
    return {
      ...
      next_follow_up_at:
        nextFollowUpCount >= 3
          ...
          : addDaysAsDateString(
              now,
              source === 'first_90_days' || source === 'linkedin_headline' || source === 'cv_checklist' || source === 'interview_prep'
                ? 5
                : nextFollowUpCount === 1
                  ? 6
                  : 2
  ...
  return {
    ...
    next_follow_up_at: addDaysAsDateString(
      now,
      source === 'first_90_days' || source === 'linkedin_headline' || source === 'cv_checklist' || source === 'interview_prep' ? 5 : 4
    ),
  };
}
