...
export const CLIENT_CV_SERVICE_SLUGS = ['cv-revamp', 'cover-letter', 'linkedin', 'bundle'] as const;
...
export const SESSION_DEBRIEF_FIELDS = [
  ...
  {
    key: 'interviewStoryEvidence',
    label: 'Interview story evidence',
    prompt: 'For Glow Up VIP, capture a concrete situation, responsibility, actions, and known outcome. Leave unsupported details for confirmation.',
...
export function getSessionDebriefFields(serviceSlug?: ClientStrategyServiceSlug) {
  return SESSION_DEBRIEF_FIELDS.filter((field) => (
    !('serviceSlug' in field) || !serviceSlug || field.serviceSlug === serviceSlug
  ));
}
...
export function countCompletedDebriefFields(
  debrief: SessionDebrief,
  serviceSlug?: ClientStrategyServiceSlug,
) {
  return getSessionDebriefFields(serviceSlug).filter((field) => debrief[field.key].trim()).length;
}

export function getClientStrategyPlanLabel(serviceSlug: ClientStrategyServiceSlug) {
  return serviceSlug === 'career-clarity'
    ? 'Career development plan · First 14 Days'
    : 'Career development plan · First 30 Days';
}
